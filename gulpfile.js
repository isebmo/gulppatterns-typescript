 ...
 */
gulp.task('tsc', ['tsc-client', 'tsc-server'], function (done) {
    done();
});

gulp.task('tsc-client', function (done) {
    runTSC(config.ts.clientts, config.ts.clientts2jsOut, done);
});

gulp.task('tsc-server', function (done) {
    //return runTSC('src/server');
    done();
});

function runTSC(directoryIN, directoryOUT, done) {
    log('runTSC');
    log('Cleaning: ' + directoryOUT);
    del.sync(directoryOUT);
    var sourceTsFiles = []
        .concat(directoryIN)
        .concat(config.ts.typings);

    var tsResult = gulp.src(sourceTsFiles)
        .pipe($.sourcemaps.init())
        .pipe($.typescript($.typescript.createProject({
            "target": "es5",
    ...
    tsResult.dts.pipe(gulp.dest(directoryOUT));
    tsResult.js
        .pipe($.sourcemaps.write('.'))
        .pipe(gulp.dest(directoryOUT))
        .on('end', function () {
            done();
        })
}
 ...
 * @return {Stream}
 */
gulp.task('jshint-codestyle', function () {
    log('Analyzing source with JSHint and JSCS');
    var withoutLib = config.js.concat('!' + config.clientApp + '**/dygraph-combined.js');
    return gulp
        .src(withoutLib)
 ...
 * @return {Stream}
 */
gulp.task('styles', ['clean-styles', 'prepare-styles'], function () {
    log('Compiling Less --> CSS');

    return gulp
        .src([config.less, config.bootstrap.less])
        .pipe($.plumber()) // exit gracefully if something fails after this
        .pipe($.if(args.sourcemaps, $.sourcemaps.init()))
        .pipe($.less())
        //        .on('error', errorLogger) // more verbose and dupe output. requires emit.
        .pipe($.autoprefixer({browsers: ['last 2 version', '> 5%', 'Firefox 24']}))
...
/**
 * Import less variables from application to bower's bootstrap repository
 */
gulp.task('prepare-styles', function () {
    return gulp.src(config.lessVariables)
        .pipe($.plumber())
        .pipe(gulp.dest(config.bootstrap.lessDirectory));
});
 ...
 */
gulp.task('build', ['tsc', 'optimize', 'images', 'fonts'], function () {
 ...
 * @return {Stream}
 */
gulp.task('test', ['jshint-codestyle', 'tsc', 'templatecache'], function (done) {
...
/**
 * serve the dev environment - Proxy activé
 * --debug-brk or --debug
 * --nosync
 */
gulp.task('serve-dev-proxy', ['inject'], function () {
    /* Activation du mode proxy pour NodeJS.
     Voir la fonction getNodeOptions */
...
function serve(isDev, specRunner) {
    ...
    return $.nodemon(nodeOptions)
        .on('restart', ['jshint-codestyle'], function (ev) {
...
function getNodeOptions(isDev) {
    return {
        ...
        env: {
            ...
            'PROXY_ENABLED': config.enableProxy
        },
        watch: [config.server]
    };
}
...
function startBrowserSync(isDev, specRunner) {
    ...
    log('Starting BrowserSync on port ' + port + ' modeDev: ' + isDev);
    ...
    gulp.watch(config.ts.clientts, ['tsc-client'], ['browserSyncReload']).on('change', changeEvent);
    ...
    var options = {
        ...
        logFileChanges: true,
        logLevel: 'debug',
        logPrefix: 'starterkit-javascript',
        notify: true,
        reloadDelay: 1000
    };
    if (specRunner) {
        ...
        options.startPath = 'dev-home-page.html';
    }
 ...
 * Create a visualizer report
 */
gulp.task('serve-reports', ['plato', 'test'], function (done) {
    gulp.src('./report/plato/index.html')
        .pipe($.open());
    gulp.src('./report/coverage/report-html/index.html')
        .pipe($.open());
});
